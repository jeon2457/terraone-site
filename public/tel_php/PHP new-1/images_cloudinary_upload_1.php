        ...
        <div class="card-container">
            ...
            <?php else: ?>
                <?php foreach ($images as $img): ?>
                    <div class="card">
                        <img src="<?php echo $img['secure_url']; ?>" alt="이미지">
                        <label>
                            <input type="checkbox" name="selected_images[]" value="<?php echo $img['secure_url']; ?>">
                            선택
                        </label>
                        <p>🔗 <?php echo basename($img['public_id']); ?></p>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
        ...
        <div class="link-input-section">
            ...
            <div class="nav-buttons">
                <button type="button" class="btn-navigation btn-first">
                    <a href="images_cloudinary_gallery_1.php">⏪ Cloudinary 갤러리로 가기(전용방)</a>
                </button>
            </div>
...
function copyLinks() {
    if (linkInput.value) {
        linkInput.select();
        // 모바일 환경에서의 복사 호환성을 위해 navigator.clipboard.writeText를 사용합니다.
        navigator.clipboard.writeText(linkInput.value).then(() => {
            alert('✅ 링크가 복사되었습니다!');
        }).catch(err => {
            // 구형 브라우저 또는 보안 문제 발생 시 대체
...
// 이미지 DB에 저장 버튼 클릭 함수 (참고용)
function saveToDB() {
    if (linkInput.value) {
        const links = linkInput.value.split('\n').filter(link => link.trim() !== '');
        
        if (links.length > 0) {
            // 실제 동작: 선택된 링크를 가지고 DB 저장 처리 파일로 이동합니다.
            const dbSaveUrl = '1/new_terraone_php/input_upload.php?urls=' + encodeURIComponent(linkInput.value);
            
            alert(`🔗 다음 ${links.length}개의 이미지 링크를 DB 저장 파일로 전송합니다.`);
            window.location.href = dbSaveUrl; 

        } else {
            alert('⚠️ 저장할 이미지를 선택해주세요.');
        }
    } else {
        alert('⚠️ 선택된 이미지가 없습니다.');
    }
