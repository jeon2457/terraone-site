...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($is_leader) {
        $stmt_tel = $pdo->prepare("SELECT tel FROM tel WHERE tel != '' ORDER BY name ASC");
        $stmt_tel->execute();
        $numbers = $stmt_tel->fetchAll(PDO::FETCH_COLUMN);
        if (!empty($numbers)) $sms_2 = implode(",", $numbers);
    }
    ...
    try {
        ...
        // 신규 회원 삽입
        $stmt = $pdo->prepare("INSERT INTO tel (id,password,name,tel,addr,remark,sms,sms_2,user_level) VALUES (:id,:password,:name,:tel,:addr,:remark,:sms,:sms_2,:user_level)");
        $stmt->execute([
            ':id'=>$id, ':password'=>$hashedPassword, ':name'=>$name,
            ':tel'=>$tel, ':addr'=>$addr, ':remark'=>$remark,
            ':sms'=>$sms, ':sms_2'=>$sms_2, ':user_level'=>$user_level
        ]);
