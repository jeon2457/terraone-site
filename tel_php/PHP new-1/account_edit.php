...
if (isset($_POST['delete_income'])) {
    $id = intval($_POST['delete_income']);
    $stmt = $pdo->prepare("DELETE FROM income_table WHERE id=?");
    $stmt->execute([$id]);
    header("Location: " . $_SERVER['PHP_SELF'] . "?month=" . $_POST['current_month']);
    exit;
}

if (isset($_POST['delete_expense'])) {
    $id = intval($_POST['delete_expense']);
...
.custom-table th:nth-child(6), .custom-table td:nth-child(6) { width: 100px; }
        ...
        <table class="table custom-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <button class="btn-edit" onclick="editIncome(<?=$item['id']?>, '<?=addslashes($item['date'])?>', '<?=addslashes($item['category'])?>', '<?=addslashes($item['description'])?>', <?=$item['amount']?>)">편집</button>
                        <form method="post" style="display:inline;" onsubmit="return confirm('정말 삭제하시겠습니까?');">
                            ...
                            <button type="submit" name="delete_income" value="<?=$item['id']?>" class="btn-delete">삭제</button>
                        </form>
                    </td>
                </tr>
                <?php endforeach; ?>
            ...
            <tfoot>
                <tr class="summary-row">
                    <td colspan="4" class="summary-label-income">월수입 합계</td>
                    <td colspan="2" class="summary-value summary-label-income"><?=number_format($monthlyIncome)?>원</td>
                </tr>
                <tr class="summary-row">
                    <td colspan="4" class="summary-label-income">1~<?=$currentMonth?>월 월수입 누계</td>
        ...
        <table class="table custom-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="amount-column"><?=number_format($item['amount'])?>원</td>
                    <td>
                        <button class="btn-edit" onclick="editExpense(<?=$item['id']?>, '<?=addslashes($item['date'])?>', '<?=addslashes($item['category'])?>', '<?=addslashes($item['description'])?>', <?=$item['amount']?>)">편집</button>
                        <form method="post" style="display:inline;" onsubmit="return confirm('정말 삭제하시겠습니까?');">
                            <input type="hidden" name="current_month" value="<?=$currentMonth?>">
                            <button type="submit" name="delete_expense" value="<?=$item['id']?>" class="btn-delete">삭제</button>
                        </form>
                    </td>
                </tr>
                <?php endforeach; ?>
            ...
            <tfoot>
                <tr class="summary-row">
                    <td colspan="4" class="summary-label-expense">월지출 합계</td>
                    <td colspan="2" class="summary-value summary-label-expense"><?=number_format($monthlyExpense)?>원</td>
                </tr>
                <tr class="summary-row">
                    <td colspan="4" class="summary-label-expense">1~<?=$currentMonth?>월 월지출 누계</td>
...
<div class="modal fade" id="editIncomeModal" tabindex="-1" aria-labelledby="editIncomeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editIncomeModalLabel">수입 편집</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form method="post" id="editIncomeForm">
                    ...
                    <label>일자</label>
                    <input type="datetime-local" name="date" id="income_date" required>
                    
                    <label>항목</label>
                    <input type="text" name="category" id="income_category" required>
                    
                    <label>비고</label>
                    <textarea name="description" id="income_description" rows="3"></textarea>
                    
                    <label>금액</label>
                    <input type="number" name="amount" id="income_amount" required>
                    
                    <div style="text-align:right; margin-top:15px;">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">취소</button>
                        <button type="submit" name="edit_income" class="btn btn-primary">저장</button>
                    </div>
                </form>
            </div>
        </div>
...
<div class="modal fade" id="editExpenseModal" tabindex="-1" aria-labelledby="editExpenseModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editExpenseModalLabel">지출 편집</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form method="post" id="editExpenseForm">
                    ...
                    <label>일자</label>
                    <input type="datetime-local" name="date" id="expense_date" required>
                    
                    <label>항목</label>
                    <input type="text" name="category" id="expense_category" required>
                    
                    <label>비고</label>
                    <textarea name="description" id="expense_description" rows="3"></textarea>
                    
                    <label>금액</label>
                    <input type="number" name="amount" id="expense_amount" required>
                    
                    <div style="text-align:right; margin-top:15px;">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">취소</button>
                        <button type="submit" name="edit_expense" class="btn btn-primary">저장</button>
                    </div>
                </form>
            </div>
        </div>
...
function editIncome(id, date, category, description, amount) {
    document.getElementById('income_id').value = id;
    document.getElementById('income_date').value = date.replace(' ', 'T');
...
function editExpense(id, date, category, description, amount) {
    document.getElementById('expense_id').value = id;
    document.getElementById('expense_date').value = date.replace(' ', 'T');
