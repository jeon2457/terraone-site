...
<head>
  ...
  <style>
    ...
    .btn-nav { width: 100%; padding: 15px; border-radius: 12px; border: 2px solid var(--primary-color); background: white; color: var(--primary-color); font-weight: 700; font-size: 1.1rem; transition: all 0.3s; cursor: pointer; text-align:center; display:inline-block; text-decoration:none; }
    .btn-nav:hover { background: var(--bg-gradient); color: white; transform: translateY(-2px); }
        ...
        <?php if ($images): ?>
          <?php foreach ($images as $index => $img): ?>
            ...
            <tr data-month="<?= $month ?>">
              ...
              <td>
                <?php if (!empty($url)): ?>
                <a class="btn-download" href="download_url.php?url=<?= urlencode($url) ?>">
                   ⬇️이미지저장
                </a>
                <?php else: ?>
