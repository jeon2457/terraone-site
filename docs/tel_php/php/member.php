  ...
  // 똑같은 비밀번호를 사용해도 코드값이 다르다!
   public function input($arr) { //배열로 처리
        try {
            ...
            VALUES (:id, :name, :password, :tel, :email, :zipcode, :addr1, :addr2, :photo, NOW(), :ip, :level)"; // level -> :level 로 수정
            ...
            $stmt->bindParam(':zipcode', $arr['zipcode'], PDO::PARAM_INT); //
            ...
            $stmt->bindParam(':level', $arr['level'], PDO::PARAM_INT);
